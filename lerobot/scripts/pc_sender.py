        ...
        try:
            while True:
                ...
                try:
                    ...
                    if "event" in msg:
                        print(f"[EVENT SENT] {msg}", end="\r")
                    else:
                        arm_keys = sum(1 for k in msg if ".pos" in k)
                        print(f"Sent: Vel={msg['base.linear_velocity']:.1f} | ArmKeys={arm_keys}", end='\r')
                except zmq.Again:
                    # Timeout 발생 시 재연결
                    self.socket.close()
                    self.socket = self.context.socket(zmq.REQ)
